...
p_i_d_Value_TypeDef PID_Value_Position_speed[4] = {{ 17.0725 , 10.0081 , 0.0 },
												   { 17.0725 , 10.0081 , 0.0 },
												   { 17.0725 , 10.0081 , 0.0  },
												   { 17.0725 , 10.0081 , 0.0  }};	//位置控制速度环pid参数大小
#else								/*增量式P，I，D参数大小*/
...
p_i_d_Value_TypeDef PID_Value_Speed[4] = {{ 45 ,3 ,0},
										  { 45 ,3 ,0},//150.0 , 0.22 , 4 
										  { 45 ,3, 0},
										  { 45 ,3, 0}};	//速度控制pid参数大小
#else						/*增量式P，I，D参数大小*/
...
int32_t Pid_control( PID_TypeDef* PID, float Feedback_value, unsigned char Mode )
{
	...
	if (Mode == 0)
	{
		if ( PID->SumError > 1000)//积分限幅
		{
			PID->SumError = 1000;
		}
		else if (PID->SumError < -1000)
		{
			PID->SumError = -1000;
		}
