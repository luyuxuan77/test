...
GPIOPinWrite(uint32_t ui32Port, uint8_t ui8Pins, uint8_t ui8Val)
{
    ...
    // Check the arguments.
    //
    ASSERT(_GPIOBaseValid(ui32Port));  // 检查参数

    //
    // Write the pins.
    //
    HWREG(ui32Port + (GPIO_O_DATA + (ui8Pins << 2))) = ui8Val;  // 写入引脚，低电平为0，高电平为引脚名
}
...
GPIOPinTypeGPIOInput(uint32_t ui32Port, uint8_t ui8Pins)
{
    ...
    // Make the pin(s) be inputs.
    //
    GPIODirModeSet(ui32Port, ui8Pins, GPIO_DIR_MODE_IN);  // 设置外设模式为输入

    //
    // Set the pad(s) for standard push-pull operation.
    //
    GPIOPadConfigSet(ui32Port, ui8Pins, GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_STD);  // 设置外设基地址，外设引脚，输出驱动强度，输出模式
}
...
GPIOPinTypeGPIOOutput(uint32_t ui32Port, uint8_t ui8Pins)
{
    ...
    //
    GPIODirModeSet(ui32Port, ui8Pins, GPIO_DIR_MODE_OUT);  // 设置外设模式为输出
}
