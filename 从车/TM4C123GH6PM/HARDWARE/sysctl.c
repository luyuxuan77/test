...
#endif
// 延时 SysCtlClockGet()*（XX）*三个指令周期的时间(s)，一个指令周期为1/XXMHz(XX为时钟频率)
// 所以 SysCtlClockGet()*1/3*3/40MHz(s)，通过SysCtlClockGet()可以得到时钟频率也是40MHz，所以就是延时1s
...
//SYSCTL_USE_PLL 表示系统时钟使用锁相环达到400Mhz。
//SYSCTL_XTAL_16MHZ 表示外部晶振是16MHz。
//SYSCTL_OSC_MAIN 表示时钟使用主振荡器。
//*****************************************************************************
// 若使用PLL倍频，则初始时钟为400MHz
// 选择使用SYSDIV则需要先除2
// 然后根据分频系数进行分频
// 例如：“SYSCTL_SYSDIV_4 | SYSCTL_USE_PLL | SYSCTL_XTAL_16MHZ | SYSCTL_OSC_MAIN”
// 则时钟为 400 / 2 / 4 = 50（MHz）
// 时钟频率显示 UARTprintf("Tiva C Series @ %u MHz\n\n", MAP_SysCtlClockGet() / 1000000);
void
...
void
SysCtlPWM1ClockSet(uint32_t ui32Config)
{
    //
    // Check the arguments.
    //
    ...
    // Check that there is a PWM block on this part.
    //
    ASSERT(HWREG(SYSCTL_DC1) & (SYSCTL_DC1_PWM1));
    ...
    //
    HWREG(SYSCTL_RCC) = ((HWREG(SYSCTL_RCC) &
                          ~(SYSCTL_RCC_USEPWMDIV | SYSCTL_RCC_PWMDIV_M | SYSCTL_DC1_PWM1)) |
                         ui32Config);
}
