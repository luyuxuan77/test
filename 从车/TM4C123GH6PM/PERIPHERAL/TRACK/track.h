...
#define Grayscale_Sensor_Digital	(unsigned char)(PE2 << 6) | (PD3 << 5) | (PD2 << 4) | (PE5 << 3) | (PE4 << 2) | (PB3 << 1) | (PB2 << 0)	//灰度传感器数字量计算


#define PB2 						GPIOPinRead(GPIO_PORTB_BASE, GPIO_PIN_2) / GPIO_PIN_2
#define PB3 						GPIOPinRead(GPIO_PORTB_BASE, GPIO_PIN_3) / GPIO_PIN_3
#define PE4 						GPIOPinRead(GPIO_PORTE_BASE, GPIO_PIN_4) / GPIO_PIN_4
#define PE5 						GPIOPinRead(GPIO_PORTE_BASE, GPIO_PIN_5) / GPIO_PIN_5
...
extern float sensor_weights_inner[NUM_SENSORS];
...
// 外部声明卡尔曼滤波器变量
extern KalmanFilter grayscale_kalman;
...
void Track_Angle_Distance_Control(float speed);
