...
//! values:
//!
//! - \b TIMER_CFG_ONE_SHOT - Full-width one-shot timer单次减计数模式
//! - \b TIMER_CFG_ONE_SHOT_UP - Full-width one-shot timer that counts up
//!   instead of down (not available on all parts)单次加计数模式
//! - \b TIMER_CFG_PERIODIC - Full-width periodic timer连续减计数模式
//! - \b TIMER_CFG_PERIODIC_UP - Full-width periodic timer that counts up
//!   instead of down (not available on all parts)连续加计数模式
//! - \b TIMER_CFG_RTC - Full-width real time clock timer实时时钟模式
//! - \b TIMER_CFG_SPLIT_PAIR - Two half-width timers分裂为一对
//!
...
//! and \e ui32Config:
//!
//! - \b TIMER_CFG_A_ONE_SHOT - Half-width one-shot timer定时器A单次减计数
//! - \b TIMER_CFG_A_ONE_SHOT_UP - Half-width one-shot timer that counts up
//!   instead of down (not available on all parts)定时器A单次加计数
//! - \b TIMER_CFG_A_PERIODIC - Half-width periodic timer定时器A连续减计数
//! - \b TIMER_CFG_A_PERIODIC_UP - Half-width periodic timer that counts up
//!   instead of down (not available on all parts)定时器A连续加计数
